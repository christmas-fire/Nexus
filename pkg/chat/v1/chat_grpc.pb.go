...
func (c *chatServiceClient) GetChatHistory(ctx context.Context, in *GetChatHistoryRequest, opts ...grpc.CallOption) (ChatService_GetChatHistoryClient, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ChatService_ServiceDesc.Streams[0], ChatService_GetChatHistory_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &chatServiceGetChatHistoryClient{ClientStream: stream}
...
type ChatService_GetChatHistoryClient interface {
	Recv() (*Message, error)
	grpc.ClientStream
}

type chatServiceGetChatHistoryClient struct {
	grpc.ClientStream
}

func (x *chatServiceGetChatHistoryClient) Recv() (*Message, error) {
	m := new(Message)
...
func _ChatService_GetChatHistory_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(GetChatHistoryRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(ChatServiceServer).GetChatHistory(m, &chatServiceGetChatHistoryServer{ServerStream: stream})
}

type ChatService_GetChatHistoryServer interface {
	Send(*Message) error
	grpc.ServerStream
}

type chatServiceGetChatHistoryServer struct {
	grpc.ServerStream
}

func (x *chatServiceGetChatHistoryServer) Send(m *Message) error {
...
var ChatService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "nexus.chat.v1.ChatService",
